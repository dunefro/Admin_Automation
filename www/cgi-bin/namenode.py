...
""")
f.close()
sp.getoutput(f"sudo ssh root@{ip} mkdir /master")
...
sp.getoutput(f"sudo ssh root@{ip} 'echo Y | hadoop namenode -format'")
sp.getoutput(f"sudo ssh root@{ip} hadoop-daemon.sh start namenode")
x=sp.getoutput("sudo ssh root@{ip} jps | grep NameNode")
if x=='':
  print("Unsuccessful")
else:
  print("Successful")
